...
class Weibo(object):
    def __init__(self, user_id, filter=0, pic_download=0):
        ...
        self.conn = pymysql.connect(host='cdb-3d8zvldt.gz.tencentcdb.com', port=10022, user='weibo_crawler',
                                    password='weibo_crawler1234', database='crawler_weibo')
    ...
    def standardize_date(self, created_at):
        ...
        if u"刚刚" in created_at:
            ...
            created_at = (datetime.now() - minute).strftime("%Y-%m-%d %H:%M:00")
        elif u"小时" in created_at:
            hour = created_at[:created_at.find(u"小时")]
            hour = timedelta(hours=int(hour))
            created_at = (datetime.now() - hour).strftime("%Y-%m-%d %H:%M:00")
        elif u"昨天" in created_at:
            day = timedelta(days=1)
            created_at = (datetime.now() - day).strftime("%Y-%m-%d %H:%M:00")
        elif created_at.count('-') == 1:
            year = datetime.now().strftime("%Y")
            created_at = year + "-" + created_at + ' 0:0:0'
        return created_at
    ...
    def parse_weibo(self, weibo_info):
        ...
        ret = self.standardize_info(weibo)
        return ret
    ...
    def get_one_weibo(self, info):
        ...
        weibo['text'] = weibo['text'].replace("'", '').replace("'", '')
        if weibo.get('retweet') is not None:
            weibo['retweet']['text'] = weibo['retweet']['text'].replace("'", '').replace("'", '')
        return weibo
    ...
    def save_to_mysql(self, wb):
        if wb.get('retweet') is not None:
            # 转发，回复
            sql_str = 'insert into weibo_detail(id,user,user_id,content,raw_pic_url,location,date,source,star_num,comment_num,forawrd_num,topic,referred_user,raw_user_id,raw_user,rawl_content_id,raw_content,raw_raw_pic_url,raw_content_location,raw_date,raw_content_source,raw_star_num,raw_comment_num,raw_forward_num,raw_topic,raw_referred_user) values ("%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s")' % (
                wb['id'], wb['screen_name'], self.user_id, wb['text'], wb['pics'], wb['location'], wb['created_at'],
                wb['source'],
                wb['attitudes_count'], wb['comments_count'], wb['reposts_count'], wb['topics'], wb['at_users'],
                wb['retweet']['user_id'], wb['retweet']['screen_name'], wb['retweet']['id'], wb['retweet']['text'],
                wb['retweet']['pics'], wb['retweet']['location'], wb['retweet']['created_at'],
                wb['retweet']['source'],
                wb['retweet']['attitudes_count'], wb['retweet']['comments_count'], wb['retweet']['reposts_count'],
                wb['retweet']['topics'], wb['retweet']['at_users'])
            ...
            sql_str = "insert into weibo_detail(id,user,user_id,content,raw_pic_url,location,date,source,star_num,comment_num,forawrd_num,topic,referred_user) values ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                wb['id'], wb['screen_name'], self.user_id, wb['text'], wb['pics'], wb['location'], wb['created_at'],
                wb['source'],
                wb['attitudes_count'], wb['comments_count'], wb['reposts_count'], wb['topics'], wb['at_users'],)
            cursor = self.conn.cursor()
